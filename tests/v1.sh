...
## v1 배포 테스트
test_maketx "[v1] Deploying (from: admin)" \
 "
  gnokey maketx addpkg \
    -pkgpath '$V1_PACKAGE_PATH' \
    -pkgdir '$V1_CONTRACT_PATH' \
    -chainid '$CHAIN_ID' \
 ...
 test_maketx "[v1] Mint(admin, 0) (from: test1) [fail]" \
 "
  gnokey maketx call \
    -pkgpath '$V1_PACKAGE_PATH' \
    -chainid '$CHAIN_ID' \
...
test_maketx "[v1] TransferFrom(test1, test2, 3) (from: admin) [fail]" \
 "
  gnokey maketx call \
    -pkgpath '$V1_PACKAGE_PATH' \
    -chainid '$CHAIN_ID' \
...
test_maketx "[v1] Burn(7) (from: test1) [fail]" \
 "
  gnokey maketx call \
    -pkgpath '$V1_PACKAGE_PATH' \
    -chainid '$CHAIN_ID' \
...
## TokenIdsOf(admin)
test_query "[v1] TokenIdsOf(admin)" \
 "gnokey query vm/qeval -data 'gno.land/r/test/v1.TokenIdsOf(\"$ADMIN\")'" \
 '(slice[("1" string),("2" string),("6" string)] []string)'

## TokenIdsOf(test1)
test_query "[v1] TokenIdsOf(test1)" \
 "gnokey query vm/qeval -data 'gno.land/r/test/v1.TokenIdsOf(\"$TEST1\")'" \
 '(slice[("0" string),("4" string),("5" string)] []string)'

## TokenIdsOf(test2)
test_query "[v1] TokenIdsOf(test2)" \
 "gnokey query vm/qeval -data 'gno.land/r/test/v1.TokenIdsOf(\"$TEST2\")'" \
 '(slice[("3" string),("8" string)] []string)'
